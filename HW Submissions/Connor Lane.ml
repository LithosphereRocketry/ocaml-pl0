...
*)
let lex_token (s : string) : token = match s with
  | "+" -> Plus
  | "-" -> Minus
  | "*" -> Times
  | "/" -> Divide
  | s -> match float_of_string_opt s with
    | None -> invalid_arg "The symbol %s is not understood by the calculator" s
    | Some x -> Num x
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  ...
  | Plus -> (match stack with
      | x :: y :: u -> (y +. x) :: u
      | _ -> failwith "You tried to add less than 2 numbers") 
  | Minus -> (match stack with
      | x :: y :: u -> (y -. x) :: u
      | _ -> failwith "You tried to subtract less than 2 numbers")
  | Times -> (match stack with
      | x :: y :: u -> (y *. x) :: u
      | _ -> failwith "You tried to multiply less than 2 numbers")
  | Divide -> (match stack with
      | x :: y :: u -> (y /. x) :: u
      | _ -> failwith "You tried to divide less than 2 numbers")
  | Num x -> x :: stack
...
let eval_rpn (input : token list) : float = 
  List.nth (List.fold_left eval_rpn_helper [] input) 0

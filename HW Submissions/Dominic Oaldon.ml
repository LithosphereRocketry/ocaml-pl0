...
let lex_token : string -> token = function 
  ...
  | "/" -> Divide
  | "*" -> Multiply
  | other -> try Num (float_of_string other) with 
    | _ -> invalid_arg "no"
...
*)
let eval_rpn_hp (stack : float list) : float list = 
    match stack with
    | (head::head2::tail) -> [head +. head2] @ tail
    | (head::tail) -> failwith "no"
    | [] -> failwith "no"

let eval_rpn_hmi (stack : float list) : float list = 
    match stack with
    | (head::head2::tail) -> [head2 -. head] @ tail
    | (head::tail) -> failwith "no"
    | [] -> failwith "no"

let eval_rpn_hmu (stack : float list) : float list = 
    match stack with
    | (head::head2::tail) -> [head *. head2] @ tail
    | (head::tail) -> failwith "no"
    | [] -> failwith "no"

let eval_rpn_hd (stack : float list) : float list = 
    match stack with
    | (head::head2::tail) -> [head2 /. head] @ tail
    | (head::tail) -> failwith "no"
    | [] -> failwith "no"
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  match input with
  | Num i -> [i] @ stack
  | Plus -> (eval_rpn_hp stack)
  | Minus -> (eval_rpn_hmi stack)
  | Multiply -> (eval_rpn_hmu stack)
  | Divide -> (eval_rpn_hd stack)
...
*)
(* let rec erpn (input : token list) : float list =
  List.fold_left (eval_rpn_helper  *)

let rec rpnh (stack1 : float list) (stack2 : token list) : float =
  match stack2 with
  | (h1::tail) -> (rpnh (eval_rpn_helper stack1 h1) tail)
  | [] -> match stack1 with 
    | (s1::t) -> s1
    | [] -> 0.

...
let lex_token (s : string) : token = 
  ...
  | "/" -> Divide
  | "?" -> Print
  | _ -> try Num (float_of_string s) with Failure _ -> invalid_arg "Give a number or operation"
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  match input with
  | Num value -> value :: stack
  | Print -> print_string (String.concat " " (List.map string_of_float (List.rev stack))); 
      print_string "\n" ; 
      stack
  | _ -> (match stack with 
      | (a::b::c) -> (match input with
          | Plus -> (b +. a) :: c
          | Minus -> (b -. a) :: c
          | Times -> (b *. a) :: c
          | Divide ->  (b /. a) :: c
          | _ -> failwith "Impossible")
      | _ -> failwith "Operation missing arguments")
...
let eval (s : string) : float =
  ...
  | _ -> eval_rpn (List.map (fun s -> lex_token s) (tokenize s))

...
let lex_token (s : string) : token = 
  ...
  | "*" -> Multiplication
  | "/" -> Division
  | x -> Number (Float.of_string x) 
  
           
let eval_rpn_helper_helper (func: float -> float -> float) (stack: float list): float list =
  match stack with
  | (first::second::rest) -> (func first second)::rest
  | (first::[]) -> [first]
  | [] -> []
(*
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  match input with
  | Addition -> eval_rpn_helper_helper (+.) stack 
  | Subtraction -> eval_rpn_helper_helper (-.) stack
  | Multiplication -> eval_rpn_helper_helper ( *. ) stack
  | Division -> eval_rpn_helper_helper (/.) stack
  | Number x -> x::stack

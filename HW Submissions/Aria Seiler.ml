...
let lex_token : string -> token = function
  ...
  | "/" -> Div
  | "?" -> DebugPrint
  | s when Option.is_some (float_of_string_opt s) -> Num (float_of_string s)
  | s -> invalid_arg ( "Unrecognized token " ^ s)
           
let do_op2 (stack : float list) (op : (float -> float -> float)) : float list =
  match stack with
  | (first :: second :: rest) -> (op second first) :: rest
  | rest -> failwith "Not enough numbers for binary operation"
...
*)
let eval_rpn_helper (stack : float list) (tok : token) : float list =
  match tok with
  | Num n -> (n :: stack)
  | Add -> do_op2 stack ( +. )
  | Sub -> do_op2 stack ( -. )
  | Mul -> do_op2 stack ( *. )
  | Div -> do_op2 stack ( /. )
  | DebugPrint -> print_endline (List.fold_left (fun str n -> str ^ (string_of_float n) ^ " ") "" stack); stack 

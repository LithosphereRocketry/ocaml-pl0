...
*)
type token =
  | BOperator of (float -> float -> float)
  | Num of float
  | EMPTY
...
let lex_token (s : string) : token = 
  ...
  | "-" -> BOperator (Float.sub)
  | "/" -> BOperator (Float.div)
  | _ -> match safe_float_of_string s with
    | Some num -> Num num 
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  match input with
  | Num float -> [float] @ stack
  | BOperator operator -> let (b::a::tail) = stack in [operator a b] @ tail
...
*)
let eval_rpn (input : token list) : float = let (a::_) = List.fold_left eval_rpn_helper [] input in a

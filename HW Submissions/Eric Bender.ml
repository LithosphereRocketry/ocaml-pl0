...
let lex_token (s : string) : token = 
  ...
  | "/" -> Op (fun a b -> a /. b)
  | "*" -> Op (fun a b -> a *. b)
  | _ -> match (Float.of_string_opt s) with
    | Some f -> Num f
    | None -> raise (Invalid_argument  s)

let rec last_two (stack: float list) : (float * float) * float list =
  match stack with 
  | [] -> raise (Failure "stack dont have enough")
  | [a;b] -> ((a, b), [])
  | (first :: rest) -> match last_two rest with
    | ((a,b), lst) -> ((a,b), (first :: lst)) 
(*
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  match input with
  | Num n -> (stack @ [n])
  | Op func -> match last_two stack with 
      ((a,b), lst) -> lst @ [(func a b)]
                     
let rec top (stack : float list) : float =
  match stack with
  | [] -> raise (Failure "stack dont have enough")
  | [f] -> f
  | (_ :: rest) -> top rest
...
*)
let rec eval_rpn (input : token list) : float = 
  top (List.fold_left eval_rpn_helper [] input)

...
let lex_token (s : string) : token = 
  ...
  | "/" -> Div
  | "?" -> Inspect
  | _ -> try Num (float_of_string s) with Failure _ -> invalid_arg ("Invalid token: " ^ s)
  
let compute (f : float -> float -> float) (stack : float list) : float list =
  match stack with
  | (first::second::rest) -> (f second first)::rest
  | _ -> failwith ("Not enough operands for operator")
(*
...
let eval_rpn_helper (stack : float list) (input : token) : float list = 
  ...
  | Mul -> compute ( *.) stack
  | Div -> compute (/.) stack
  | Inspect -> Printf.printf "Stack: [";
      List.iter (fun x -> Printf.printf " %g" x) (List.rev stack);
      Printf.printf " ]\n";
      stack
  | Num n -> n::stack
...
let eval_rpn (input : token list) : float = 
  match List.fold_left eval_rpn_helper [] input with
  | result::rest -> result
  | [] -> invalid_arg "No result on the stack"
...
let eval (s : string) : float =
  ...
  let tokens = (List.map lex_token token_list) in
  (eval_rpn tokens)

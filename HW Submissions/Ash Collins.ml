...
*)
let lex_token : string -> token = 
  function
  | "+" -> Plus
  | "-" -> Minus
  | "*" -> Times
  | "/" -> Div 
  | a when a == a -> (match Float.of_string_opt a with
      | Some(x) -> Num x
      | None -> invalid_arg "Invalid symbol")
...
*)
let eval_rpn_helper (stack : float list) : token -> float list = 
  function
  | Num x -> x :: stack
  | Plus -> (match stack with
      | (a :: b :: rest) -> (a +. b) :: rest
      | _ -> failwith "Too few arguments for +")
...
let eval_rpn (input : token list) : float = 
  ...
  | (x :: rest)-> x
  | _ -> invalid_arg "How did you do that"
